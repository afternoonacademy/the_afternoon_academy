...
export default async function PlaceOffersPage() {
  ...
  return <div className="space-y-8">
    ...
    <Card><CardHeader><CardTitle>Create and send an offer</CardTitle></CardHeader><CardContent><PlaceOfferForm families={families || []} children={children || []} slots={slots || []} /></CardContent></Card>
    <Card><CardHeader><CardTitle>Current held offers</CardTitle></CardHeader><CardContent>{offers?.length ? <div className="space-y-3">{offers.map((offer: any) => <div className="rounded-lg border p-4" key={offer.id}><div className="font-medium">{offer.child_leads?.first_name || "Child"} · {offer.parent_leads?.parent_name || "Parent"}</div><div className="mt-1 text-sm text-muted-foreground">Reference {offer.payment_reference} · {offer.status.replaceAll("_"," ")} · expires {new Intl.DateTimeFormat("en-GB",{dateStyle:"medium",timeStyle:"short"}).format(new Date(offer.expires_at))}</div></div>)}</div> : <p className="text-sm text-muted-foreground">No places are currently held by an offer.</p>}</CardContent></Card>
  </div>
}
