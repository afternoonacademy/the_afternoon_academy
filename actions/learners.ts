...
const deliverySeatActionSchema = z.object({
  deliverySeatId: z.string().uuid(),
});
const deliverySessionActionSchema = z.object({
  deliverySessionId: z.string().uuid(),
});

const weeklyDeliveryChangeSchema = z
  .object({
  ...
  .superRefine((value, ctx) => {
    if (value.learnerAction === "add" && !value.seatNumber)
      ctx.addIssue({
        code: "custom",
        message: "Choose a seat for the new learner",
      });
    if (value.learnerAction !== "none" && !value.learnerId)
      ctx.addIssue({ code: "custom", message: "Choose a learner" });
  });
...
const generatePaidMonthSchema = z.object({
  month: z.string().regex(/^\d{4}-\d{2}$/),
});

const tutorBookingSchema = z
  .object({
    learnerId: z.string().uuid().optional(),
    parentLeadId: z.string().uuid().optional(),
    teacherName: z.string().trim().min(1).max(160),
    startsAt: z.string().min(16).max(40),
    endsAt: z.string().min(16).max(40),
    note: z.string().trim().max(1000).optional(),
  })
  .refine((value) => value.learnerId || value.parentLeadId, {
    message: "Choose a learner or a parent lead",
  });
...
export async function createLearner(formData: FormData) {
  ...
  const parsed = learnerSchema.safeParse({
    ...
    parentInformationConfirmed:
      formData.get("parentInformationConfirmed") === "on",
  });

  if (!parsed.success)
    throw new Error("Please complete the required learner details");
  ...
  const { error: profileError } = await supabase
    ...
    .insert({
      ...
      parent_priorities:
        String(formData.get("parentPriorities") || "").trim() || null,
      helpful_strategies:
        String(formData.get("helpfulStrategies") || "").trim() || null,
    });
...
export async function updateLearnerDetails(formData: FormData) {
  ...
  const parsed = learnerDetailsSchema.safeParse({
    ...
    schoolContactPermissionConfirmed:
      formData.get("schoolContactPermissionConfirmed") === "on",
  });
  ...
  const hasSchoolContact = Boolean(
    details.teacherName || details.teacherEmail || details.teacherPhone,
  );
  if (hasSchoolContact && !details.schoolContactPermissionConfirmed) {
    throw new Error(
      "Confirm permission before recording school contact details",
    );
  }
  ...
  if (
    ...
    if (placementError)
      throw new Error("Could not verify the learner placement");
    if (!count) {
      throw new Error(
        "A learner linked to an enquiry becomes active when a parent accepts a confirmed session offer",
      );
    }
  }
...
export async function updateLearnerPersonalProfile(formData: FormData) {
  ...
  if (!parsed.success)
    throw new Error("Please check the personal profile details");
  const value = parsed.data;
  if (
    (value.teacherName || value.teacherEmail || value.teacherPhone) &&
    !value.schoolContactPermissionConfirmed
  )
    throw new Error(
      "Confirm permission before recording school contact details",
    );
  const { error } = await supabaseService()
    ...
    .update({
      ...
      year_group: value.yearGroup || null,
      current_school_name: value.currentSchoolName || null,
      teacher_name: value.teacherName || null,
      teacher_email: value.teacherEmail || null,
      teacher_phone: value.teacherPhone || null,
      school_contact_permission_confirmed:
        value.schoolContactPermissionConfirmed,
    })
...
export async function saveSessionPlacement(formData: FormData) {
  ...
  if (existing && ["offered", "confirmed"].includes(existing.status)) {
    throw new Error(
      "Use the placement action below; an offered or confirmed place cannot be overwritten",
    );
  }
  const { error } = await supabase.from("session_placements").upsert(
    {
      session_id: parsed.data.sessionId,
      learner_id: parsed.data.learnerId,
      status: parsed.data.status,
      fit_note: parsed.data.fitNote || null,
      created_by: user.id,
    },
...
export async function sendSessionOffer(formData: FormData) {
  ...
  if (error)
    throw new Error(error.message || "Could not send the session offer");
...
export async function acceptSessionOffer(formData: FormData) {
  ...
  if (error)
    throw new Error(error.message || "Could not confirm the session offer");
  revalidatePath("/admin/leads");
  revalidatePath("/admin/sessions");
  revalidatePath("/admin/learners");
  revalidatePath("/admin/operations");
...
export async function saveLeadSessionOffer(formData: FormData) {
  ...
  if (!parsed.success)
    throw new Error("Please choose the parent, child and intended seat");
  const supabase = supabaseService();
  ...
  if (
    childError ||
    !child ||
    child.parent_lead_id !== parsed.data.parentLeadId
  ) {
  ...
  const { error } = await supabase.from("session_offers").upsert(
    {
      ...
      learner_first_name: child.first_name,
      learner_year_group: child.school_year || null,
...
export async function sendLeadSessionOffer(formData: FormData) {
  ...
  const { error } = await supabaseService().rpc("send_lead_session_offer", {
...
export async function acceptLeadSessionOffer(formData: FormData) {
  ...
  if (!parsed.success) throw new Error("Invalid offer");
  const { error } = await supabaseService().rpc("accept_lead_session_offer", {
    p_offer_id: parsed.data.offerId,
    p_actor_id: user.id,
  });
...
export async function createTutorRoomBooking(formData: FormData) {
  const { user } = await requireAdmin();
  const parsed = tutorBookingSchema.safeParse({
  ...
  const { error } = await supabaseService()
    ...
    .insert({
      learner_id: parsed.data.learnerId || null,
      parent_lead_id: parsed.data.parentLeadId || null,
  ...
  if (error)
    throw new Error(
      error.code === "23P01"
        ? "The Tutor Room is already booked at that time"
        : "Could not save Tutor Room booking",
    );
  revalidatePath("/admin/sessions");
}
...
export async function createMonthlyDeliveryPlan(formData: FormData) {
  ...
  if (!parsed.success)
    throw new Error("Please complete the monthly table plan");
  const plan = parsed.data;
  const supabase = supabaseService();
  const { data: planRow, error: planError } = await supabase
    .from("monthly_delivery_plans")
    .insert({
      month_start: `${plan.month}-01`,
  ...
  const { data: deliverySessions, error: sessionError } = await supabase
  ...
  const learnerIds = formData
    .getAll("learnerId")
    .map((value) => String(value))
    .filter(Boolean);
  if (learnerIds.length) {
    const seats = deliverySessions.flatMap((deliverySession) =>
      learnerIds.map((learnerId, index) => ({
    ...
    if (seatError)
      throw new Error(
        "The monthly plan was created, but its learner seats could not be added",
      );
  }
  revalidatePath("/admin/sessions");
}
...
export async function createDailyDeliverySession(formData: FormData) {
  ...
  if (!parsed.success)
    throw new Error("Please complete this table's daily plan");
  ...
  if (error)
    throw new Error("This table is already planned for the selected date");
  revalidatePath("/admin/sessions");
}
...
export async function addDeliverySeat(formData: FormData) {
  ...
  const parsed = deliverySeatSchema.safeParse({
    deliverySessionId: formData.get("deliverySessionId"),
    learnerId: formData.get("learnerId"),
    seatNumber: formData.get("seatNumber"),
  });
  if (!parsed.success)
    throw new Error("Choose a learner and an available seat");
  const { error } = await supabaseService().from("delivery_seats").insert({
    delivery_session_id: parsed.data.deliverySessionId,
    learner_id: parsed.data.learnerId,
    seat_number: parsed.data.seatNumber,
    status: "scheduled",
    updated_by: user.id,
...
export async function removeDeliverySeat(formData: FormData) {
  ...
  const parsed = deliverySeatActionSchema.safeParse({
    deliverySeatId: formData.get("deliverySeatId"),
  });
  if (!parsed.success) throw new Error("Invalid seat");
  const { error } = await supabaseService()
    .from("delivery_seats")
    .update({ status: "not_attending", updated_by: user.id })
    .eq("id", parsed.data.deliverySeatId);
...
export async function cancelDeliverySession(formData: FormData) {
  const { user } = await requireAdmin();
  const parsed = deliverySessionActionSchema.safeParse({
    deliverySessionId: formData.get("deliverySessionId"),
  });
...
export async function applyWeeklyDeliveryChange(formData: FormData) {
  ...
  const parsed = weeklyDeliveryChangeSchema.safeParse({
    ...
    focus: formData.get("focus") || undefined,
    learnerId: formData.get("learnerId") || undefined,
    learnerAction: formData.get("learnerAction") || "none",
    seatNumber: formData.get("seatNumber") || undefined,
  });
  if (!parsed.success) throw new Error("Please check the weekly change");
  const change = parsed.data;
  if (!change.teacherName && !change.focus && change.learnerAction === "none")
    throw new Error("Choose a teacher, session type or learner change");
  ...
  const { data: sessions, error: sessionError } = await supabase
    .from("delivery_sessions")
    .select("id")
    .eq("table_number", change.tableNumber)
    .eq("status", "scheduled")
    .gte("service_date", change.weekStart)
    .lte("service_date", end);
  if (sessionError) throw new Error("Could not find that week's table plans");
  if (!sessions?.length)
    throw new Error(
      "There are no planned sessions for that table in this week",
    );
  ...
  if (change.learnerAction === "remove" && change.learnerId) {
    const { error } = await supabase
      ...
      .eq("learner_id", change.learnerId)
      .eq("status", "scheduled");
  ...
  if (change.learnerAction === "add" && change.learnerId && change.seatNumber) {
    ...
    if (
      occupied?.some(
        (seat) =>
          seat.seat_number === change.seatNumber ||
          seat.learner_id === change.learnerId,
      )
    ) {
      throw new Error(
        "That learner or seat is already occupied on at least one day in the chosen week",
      );
    }
    const { error } = await supabase.from("delivery_seats").insert(
      sessionIds.map((deliverySessionId) => ({
        delivery_session_id: deliverySessionId,
        learner_id: change.learnerId,
        seat_number: change.seatNumber,
        status: "scheduled",
        updated_by: user.id,
      })),
    );
    ...
    .upsert(
      {
        ...
        sessions_per_week: parsed.data.sessionsPerWeek,
        status: "paid",
...
export async function saveStandingPlacement(formData: FormData) {
  ...
  if (!parsed.success)
    throw new Error("Please complete the standing timetable place");
...
export async function generatePaidMonth(formData: FormData) {
  const { user } = await requireAdmin();
  const parsed = generatePaidMonthSchema.safeParse({
    month: formData.get("month"),
  });
  ...
  const [
    ...
    { data: entitlements, error: paymentError },
  ] = await Promise.all([
    supabase
      .from("standing_placements")
      .select(
        "id, learner_id, weekday, table_number, starts_at, duration_minutes, teacher_name, focus, effective_from, effective_to",
      )
      .eq("status", "active"),
    supabase
      .from("learners")
    ...
    supabase
      .from("payment_entitlements")
      .select("parent_lead_id, period_start, period_end")
      .eq("status", "paid")
      .lte("period_start", end)
      .gte("period_end", start),
  ]);
  if (placementError || learnerError || paymentError)
    throw new Error("Could not load the paid timetable");
  const parentByLearner = new Map(
    (learners || []).map((learner) => [learner.id, learner.parent_lead_id]),
  );
  const paidParents = new Set(
    (entitlements || []).map((entitlement) => entitlement.parent_lead_id),
  );
  const eligible = (placements || []).filter((placement) => {
    const parentLeadId = parentByLearner.get(placement.learner_id);
    return (
      parentLeadId &&
      paidParents.has(parentLeadId) &&
      placement.effective_from <= end &&
      (!placement.effective_to || placement.effective_to >= start)
    );
  });

  for (const placement of eligible) {
    for (const serviceDate of datesInMonthForWeekday(
      month,
      placement.weekday,
    )) {
      const { data: session, error: existingError } = await supabase
        ...
        .eq("table_number", placement.table_number)
        .maybeSingle();
      ...
      if (!sessionId) {
        ...
        if (createError || !created)
          throw new Error("Could not create a paid delivery date");
      ...
      if (!seat) {
        const { count } = await supabase
          ...
          .eq("status", "scheduled");
        if ((count || 0) >= 6)
          throw new Error("A paid table would exceed six seats");
        const { error: seatError } = await supabase
          .from("delivery_seats")
          .insert({
...
export async function saveWeeklyTableTemplate(formData: FormData) {
  ...
  if (!parsed.success)
    throw new Error("Please complete the weekly table details");
  const supabase = supabaseService();
...
export async function savePaidWeeklyPlace(formData: FormData) {
  ...
  const { data: learner, error: learnerError } = await supabase
    ...
    .eq("status", "active")
    .single();
  if (learnerError || !learner?.parent_lead_id)
    throw new Error("That learner needs a linked parent record");
  ...
  if (paymentError || !payment)
    throw new Error("Could not record the family payment period");
  ...
  if (childPaymentError)
    throw new Error("Could not give the child paid eligibility");
  const { error: closeError } = await supabase
    .from("standing_placements")
    .update({
      status: "ended",
      effective_to: parsed.data.periodStart,
      updated_by: user.id,
    })
    .eq("learner_id", parsed.data.learnerId)
    .eq("weekday", parsed.data.weekday)
    .eq("status", "active");
  if (closeError)
    throw new Error("Could not update the learner's previous weekly place");
  const { error: placementError } = await supabase
    ...
    .insert({
      ...
      teacher_name: parsed.data.teacherName || null,
      focus: parsed.data.focus,
      effective_from: parsed.data.periodStart,
      created_by: user.id,
      updated_by: user.id,
    });
  if (placementError)
